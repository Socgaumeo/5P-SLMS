#!/usr/bin/env python3
"""Test script đơn giản để debug parsing drivers SQL"""
...
if matches:
    for i, columns_str in enumerate(matches[:1], 1):
        ...
        # Tìm full INSERT statement
        full_pattern = rf"INSERT\s+INTO\s+drivers\s*\({re.escape(columns_str)}\)\s*SELECT.*?WHERE\s+NOT\s+EXISTS"
        ...
        if full_match:
            ...
            # Tìm SELECT values
            # Pattern cần xử lý newline và whitespace - sử dụng \s+ thay vì \s*
