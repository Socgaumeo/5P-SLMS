...
Sử dụng Supabase REST API với RPC function
"""
...
class VendorSurchargesManager:
    ...
    def execute_sql_via_rpc(self, sql_query: str):
        """
        Thực thi SQL thông qua Supabase RPC function
        Lưu ý: Cần tạo RPC function 'exec_sql' trên Supabase trước
        """
        try:
            result = self.supabase.rpc('exec_sql', {'sql_query': sql_query}).execute()
            return result
        except Exception as e:
            print(f"❌ Lỗi khi thực thi SQL qua RPC: {str(e)}")
            raise
    
    def create_vendor_surcharges_table(self):
        """
        Tạo bảng vendor_surcharges với cấu trúc đúng
        Lưu ý: Phải thực hiện thủ công trên Supabase Dashboard
        """
        print("\n" + "="*60)
        print("HƯỚNG DẪN TẠO BẢNG VENDOR_SURCHARGES")
        print("="*60)
        
        print("\n⚠️  Supabase Python SDK không hỗ trợ thực thi SQL trực tiếp.")
        print("Bạn cần thực hiện các bước sau trên Supabase Dashboard:")
        
        print("\n1️⃣  Truy cập: https://app.supabase.com/project/vpmsytbbsxmtdicnkytv")
        print("2️⃣  Vào **SQL Editor**")
        print("3️⃣  Copy và chạy SQL sau:")
...
DROP TABLE IF EXISTS public.vendor_surcharges;
...
"""
        ...
        print("\n4️⃣  Sau khi chạy SQL thành công, nhấn Enter để tiếp tục import...")
        input()
        
        print("✅ Đã nhận xác nhận. Tiếp tục import dữ liệu...")
...
def main():
    ...
    try:
        if len(sys.argv) < 2:
            print("Usage: python create_and_import_vendor_surcharges_v2.py <file_path> [vendor_name]")
            print("Example: python create_and_import_vendor_surcharges_v2.py vendor_rates/Tam_bảo_092025.xlsx 'Tam Bảo'")
            sys.exit(1)
        ...
        # Hướng dẫn tạo bảng vendor_surcharges
