#!/usr/bin/env python3
"""Test script để debug parsing drivers SQL - phiên bản 3"""
...
# Test pattern mới
insert_statements = re.findall(r"INSERT INTO drivers \((.*?)\) SELECT", sql_content, re.DOTALL | re.IGNORECASE)
    ...
    for i, columns_str in enumerate(insert_statements[:3], 1):
        ...
        if insert_pos != -1:
            ...
            if where_pos != -1:
                ...
                # Parse SELECT values
                select_pattern = r"'([^']*)',\s*'([^']*)',\s*'([^']*)',\s*'([^']*)',\s*'([^']*)',\s*\(SELECT vendor_id FROM vendors WHERE vendor_code = '([^']*)'\),\s*(TRUE|FALSE)"
